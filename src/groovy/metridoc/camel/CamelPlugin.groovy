...
class CamelPlugin {
    ...
    static <T> T getCamelService(Script self, String name, Class<T> type) {
        ...
        assert services: "services in the CamelPlugin must be set to create or retrieve camel components"
    ...
    static <T> T getServiceAndSetProperty(Script self, String propertyName, String serviceName, Class<T> type, Closure getDefault) {
        ...
        service = getDefault()
        assert service: "When retrieving a service, the passed default function 'getDefault' must " +
            "not be null"
        def services = self.binding
